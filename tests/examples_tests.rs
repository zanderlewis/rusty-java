...
fn test_example_project(example_name: &str) -> Result<(), String> {
    ...
    if !gradle_path.exists() {
        return Err(format!(
            "Gradle build directory not found for example '{}'",
            example_name
        ));
    }
...
fn verify_build_output(example_path: &Path, example_name: &str) -> Result<(), String> {
    ...
    let project_name = parse_project_name(&toml_content).ok_or_else(|| {
        format!(
            "Failed to parse project name from rsj.toml for example '{}'",
            example_name
        )
    })?;

    let version = parse_project_version(&toml_content).unwrap_or_else(|| "1.0.0".to_string());
    ...
    if !build_dir.exists() {
        return Err(format!(
            "Build directory not found for example '{}' at: {}",
            example_name,
            build_dir.display()
        ));
    }
    ...
    if !libs_dir.exists() {
        return Err(format!(
            "Libs directory not found for example '{}' at: {}",
            example_name,
            libs_dir.display()
        ));
    }
    ...
    let jar_files: Vec<_> = fs::read_dir(&libs_dir)
        .map_err(|_| {
            format!(
                "Failed to read libs directory for example '{}'",
                example_name
            )
        })?
    ...
    println!(
        "Found JAR file(s) for example '{}' at: {}",
        example_name,
        jar_files[0].path().display()
    );
    Ok(())
}
