...
fn test_gradle_wrapper() {
    ...
    let wrapper_bat = test_dir.join("rsj_build").join("gradle").join("gradlew.bat");
    let wrapper_jar_dir = test_dir.join("rsj_build").join("gradle").join("gradle").join("wrapper");
    ...
    assert!(wrapper_properties.exists(), "Gradle wrapper properties not found");
...
fn test_gradle_performance_options() {
    ...
    assert!(content.contains("org.gradle.parallel=true"), "Parallel execution not enabled");
    assert!(content.contains("org.gradle.caching=true"), "Build cache not enabled");
    assert!(content.contains("org.gradle.vfs.watch=true"), "File system watching not enabled");
...
fn setup_test_project(project_name: &str) -> Result<std::path::PathBuf, String> {
    ...
    let test_dir = Path::new(&manifest_dir).join("target").join("test_projects").join(project_name);
    ...
    let mut main_file = File::create(&main_java)
        .map_err(|e| format!("Failed to create Main.java: {}", e))?;
    ...
    let mut toml_file = File::create(&rsj_toml)
        .map_err(|e| format!("Failed to create rsj.toml: {}", e))?;
